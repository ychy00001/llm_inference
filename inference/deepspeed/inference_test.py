...
parser.add_argument("--dtype", default="int8", type=str, choices=["float32", "float16", "int8"], help="data-type")
...
world_size = int(os.getenv('WORLD_SIZE', '6'))
...
for i in range(iters):
    ...
    start = time.time()
    #input_ids = pipe.tokenizer(inputs, return_tensors="pt").input_ids.to(pipe.device)
    outputs = pipe(inputs,
